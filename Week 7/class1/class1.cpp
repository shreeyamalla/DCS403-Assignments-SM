...
/* QUEUE : linear list where elements are:  
    ...
    Variables used: 
        fromt: index of 1st item of the queue
        rear: index of last item added to the queue
    when queue empty, both front and rear values are -1
    insertion huda, rear values is incremented by 1 & said element is inserted at the new rear
    deletion huda, front position ma vayeko element is del & value of front is incremenented by 1
    first element insert huda, front & rear ko values change from -1 to 0 (only for this one time)
*/
